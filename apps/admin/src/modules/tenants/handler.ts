...
// Wire the deny-path audit hook on every operator gate so a missing or
// under-privileged operator surfaces an `operator.access.denied` row
// alongside the 401/403 response.
 ...
 * advertises `application/json` but cannot be parsed.
 */
async function readOptionalReason(req: Request): Promise<OptionalReasonResult> {
  ...
  // Peek the body so we can distinguish "no body" from "malformed JSON".
  let body: string;
  try {
    body = await req.text();
  } catch {
...
app.post(
  ...
  async (c) => {
    ...
    try {
      ...
      // Workers RPC preserves `name`/`code` but not class identity, so the
      // server worker's slug-conflict errors must be narrowed structurally.
...
// Idempotent at the service layer; both first-suspend and re-suspend resolve
// to 204.
app.post(
...
// untouched so previously revoked JWTs stay revoked.
app.post(
